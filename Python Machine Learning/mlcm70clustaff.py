...
datos = {"alan" : [2, 4],
         ...
         "zelda" : [7, 6], 
         "zoila" : [6, 7], 
         "zulema" : [6, 8]}   # ahorro y evaluacion crediticia  de 1 a 10
...
print(datos)
print("Similitud entre Alan y Alberto", -((datos["alan"] - datos["alberto"])**2).sum())    # -2  Distancia euclediana al cuadraro
print("Similitud entre Alan y Zulema", -((datos["alan"] - datos["zulema"])**2).sum())    # -32
...
np.fill_diagonal(s.values, np.min(s.values))   # Valor minimo
print("Similitus Min")
print(s)

# Matriz de Disponibilidad
d = pd.DataFrame(0, columns=datos.columns, index=datos.columns) 
print("Matriz Disponibilidad")  # Con ceros no hay evidencia
print(d)
# La responsabilidad se actualiza a partir de la similitud y la disponibilidad
print("Similitus actualizada")
print(s)
# Responsabilidad ["alan", "alberto"]

sim = s.loc["alan", "alberto"]  # Similitud alan alberto

sim_otros = s.loc["alan", ["alan", "alex", "zelda", "zoila", "zulema"]]   # similitud alan con otros no alberto
dis_otros = d.loc["alan", ["alan", "alex", "zelda", "zoila", "zulema"]]    # disponibilidad

print("Qué tan adecuado es Alberto como EJEMPLAR para Alan:", sim - max(sim_otros + dis_otros)) 

sim = s.loc["alan", "zulema"] # siilitud alan zulema
...
for i in range(10):
    factor = 0.5
    # Actualización de Responsabilidades  Igual a dos ejemplos anteriores para todos
